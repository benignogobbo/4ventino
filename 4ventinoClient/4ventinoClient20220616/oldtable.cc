...
// | with tablular formatted output                         |
// | Benigno Gobbo - INFN Trieste                           |
// | V1.0 20190904                                          |
// | V1.1 20191202 - small fix in output format             |
// +--------------------------------------------------------+
...
void dump( void ) {
  ...
  for( int i=0; i<toRead; i++ ) {
    ...
    for( int j=0; j<5; j++ ) {
      if( data[i][j] > -9999 ) {
	std::cout << "        \033[1;32m" << std::right << std::setw(6) << std::fixed << std::setprecision( precision[j] ) << data[i][j] << std::left << "\033[0m";
      }
      else {
