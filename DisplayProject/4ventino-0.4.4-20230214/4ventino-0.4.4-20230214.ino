  ...
  if( ( millis() - tm1 ) > dataUpdatePeriod && !NXDY_inSetup ) {  // slow down loop frequency
    ...
    if( client ) {
      ...
      if( strncmp( buff, "data", 4 ) == 0 ) {
        ...
        server.write( String( eepromData.eepromContent.line ).c_str() );
      }
    ...
    if( Serial.available() ) {
      ...
      else if( s.substring(0,4).compareTo( String( "line" ) ) == 0 ) {
        String line = String( eepromData.eepromContent.line );
        int availableBytes = Serial.availableForWrite();
        if( availableBytes >= line.length() ) {
          Serial.println( line.substring(0,availableBytes) );
          while( Serial.available()>0) { char r = Serial.read(); }; // clean input serial buffer
        }
      }
